...
document.getElementById("email-form").addEventListener("submit", (event) => {
  ...
  formattedTable += "Message: " + message + "\n\n";
  formattedTable += "Wishlist:\n";
  formattedTable += "+------------------+----------+---------+--------+\n";
  formattedTable += "|      Product     | Quantity |   Size  |  Total |\n";
  formattedTable += "+------------------+----------+---------+--------+\n";
  ...
  rows.forEach((row, index) => {
    if (!row.classList.contains("disabled") && index !== rows.length - 1) {
      ...
      formattedTable += `| ${name.padEnd(18)} | ${quantity.padStart(
        8
      )} | ${size.padStart(7)} | ${total.padStart(7)} |\n`;
    }
  });

  const grandTotal = document.getElementById("grand-total").textContent;
  formattedTable += "+------------------+----------+---------+--------+\n";
  formattedTable += `|   Grand Total:   |          |         | ${grandTotal.padStart(
    7
  )} |\n`;
  formattedTable += "+------------------+----------+---------+--------+\n";
